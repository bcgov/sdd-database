...
const reportOptions = [
  ...
  {
    id: "employees_by_branch_or_program_area",
    label: "Employees (by Branch or Program Area)",
  },
  { id: "employees_by_job_title", label: "Employees (by Position Title)" },
  {
    id: "workspace_holds_by_office_code_and_status",
    label: "Workspace Holds (by Office Code, Availability)",
  },
  {
    id: "unassigned_workspaces_by_office_code_and_status",
    label: "Unassigned Workspaces (by Office Code and Hold Status)",
  },
  { id: "workspaces_by_office_code", label: "Workspaces (by Office Code)" },
  {
    id: "mobile_devices_by_office_code",
    label: "Mobile Devices (by Office Code)",
  },
  { id: "mobile_devices_by_imei", label: "Mobile Devices (by IMEI)" },
  {
    id: "workstation_assets_by_asset_number",
    label: "Workstation Asset (by Asset Number)",
  },
  {
    id: "workstation_assets_by_office_code_and_model",
    label: "Workstation Asset (by Office Code and Model)",
  },
  {
...
export function ReportsModal() {
  ...
  const [workspaceAvailability, setWorkspaceAvailability] =
    useState<string>("");
  const [employeeQuery, setEmployeeQuery] = useState<string>("");
  const [selectedBranchId, setSelectedBranchId] = useState<string>("");
  const [selectedProgramAreaId, setSelectedProgramAreaId] =
    useState<string>("");
  ...
  const [selectedJobTitleReportId, setSelectedJobTitleReportId] =
    useState<string>("");
  const [branchOptions, setBranchOptions] = useState<
    Array<{ id: string; label: string }>
  >([]);
  const [allProgramAreaOptions, setAllProgramAreaOptions] = useState<
    Array<{ id: string; label: string; branchId: string }>
  >([]);
  const [jobTitleOptions, setJobTitleOptions] = useState<
    Array<{ id: string; label: string }>
  >([]);
  const [workspaceStatusOptions, setWorkspaceStatusOptions] = useState<
    Array<{ id: string; label: string }>
  >([]);
  const [workstationModelOptions, setWorkstationModelOptions] = useState<
    Array<{ id: string; label: string }>
  >([]);
    ...
    const loadFilterOptions = async () => {
      if (
        ...
        try {
          ...
          setBranchOptions(
            data.branches.map((branch: { id: number; name: string }) => ({
              id: branch.id.toString(),
              label: branch.name,
            })),
          );
          setAllProgramAreaOptions(
            data.programAreas.map(
              (programArea: {
                id: number;
                name: string;
                branch_id: number;
          ...
          setJobTitleOptions(
            data.jobTitles.map((jobTitle: { id: number; name: string }) => ({
          ...
          setError(
            e instanceof Error
              ? e.message
              : "Unable to load branch and program area options",
          );
        }
      }
      ...
      if (selectedReport === "workspace_holds_by_office_code_and_status") {
        try {
          ...
          const data = await response.json();
          setWorkspaceStatusOptions(
            data.statuses.map((status: { value: string; label: string }) => ({
              id: status.value,
              label: status.label,
            })),
          );
          ...
          setError(
            e instanceof Error
              ? e.message
              : "Unable to load workspace status options",
          );
        }
      }
      ...
      if (
        ...
        try {
          const response = await fetch(
            "/api/reports/unassigned-workspace-status-options",
          );
          if (!response.ok) {
            throw new Error(
              "Unable to load unassigned workspace status options",
            );
          }
      ...
      if (selectedReport === "workstation_assets_by_office_code_and_model") {
        try {
          ...
          const data = await response.json();
          setWorkstationModelOptions(
            data.models.map((model: { id: number; label: string }) => ({
          ...
          setError(
            e instanceof Error
              ? e.message
              : "Unable to load workstation model options",
          );
        }
      }
  ...
  const handleGenerate = async () => {
    if (selectedReport === "workstation_assets_by_asset_number") {
      const validationError = validateAssetTagField(
        assetNumber,
        "Asset number",
      );
      if (validationError) {
      ...
      const officeValidationError = validateOfficeNumberField(
        officeCode,
        "Office code",
      );
    ...
    try {
      const endpoint =
        ...
        selectedReport === "mobile_devices_by_imei"
          ...
          : selectedReport === "workspace_holds_by_office_code_and_status" ||
              selectedReport ===
                "unassigned_workspaces_by_office_code_and_status"
                  ...
                  : selectedReport === "employees_by_branch_or_program_area"
                    ...
                    : selectedReport === "employees_by_job_title"
                      ...
                      : selectedReport === "records_by_office_code"
                        ...
                        : selectedReport ===
                            "workstation_assets_by_asset_number"
                          ? `workstations-${assetNumber}.xlsx`
                          : selectedReport ===
                              "workstation_assets_by_office_code_and_model"
                            ? `workstations-${officeCode}-${modelName || "all"}.xlsx`
                            : selectedReport ===
                                "redeploy_workstation_assets_by_office_code"
                              ? `redeploy-workstations.xlsx`
                              : selectedReport ===
                                  "redeploy_mobile_assets_by_office_code"
            ...
            : selectedReport === "mobile_devices_by_imei"
              ...
              : selectedReport === "workspace_holds_by_office_code_and_status"
                ? JSON.stringify({
                    ...
                    employeeIdPopulated: workspaceAvailability || undefined,
                  })
                : selectedReport ===
                    "unassigned_workspaces_by_office_code_and_status"
                  ? JSON.stringify({
                  ...
                  : selectedReport === "employees_by_name_or_idir"
                    ...
                    : selectedReport === "employees_by_branch_or_program_area"
                      ? JSON.stringify({
                          ...
                          programAreaId: selectedProgramAreaId || undefined,
                          officeCode: selectedOfficeCode.trim() || undefined,
                          jobTitleId: selectedJobTitleId || undefined,
                          mode: "branch_or_program_area",
                        })
                      : selectedReport === "employees_by_job_title"
                        ? JSON.stringify({
                            branchId: selectedBranchId || undefined,
                            programAreaId: selectedProgramAreaId || undefined,
                            officeCode: selectedOfficeCode.trim() || undefined,
                            jobTitleId: selectedJobTitleReportId || undefined,
                            mode: "job_title",
                          })
                        : selectedReport === "records_by_office_code"
                          ? JSON.stringify({
                              officeCode: officeCode.trim().toUpperCase(),
                            })
                          : selectedReport ===
                                "redeploy_workstation_assets_by_office_code" ||
                              selectedReport ===
        ...
        >
            ...
            >
              ...
              >
                ...
                <Select
                  ...
                  value={selectedJobTitleReportId}
                  onChange={(key) => {
              ...
              >
                <label htmlFor="jobTitleProgramAreaSelect">
                  Program Area (optional)
                </label>
                <Select
                  name="jobTitleProgramAreaSelect"
                  items={visibleProgramAreaOptions}
                  value={selectedProgramAreaId}
                  onChange={(key) => {
              ...
              >
                <label htmlFor="jobTitleOfficeCodeSelect">
          ...
          {selectedReport === "workspace_holds_by_office_code_and_status" && (
            ...
            >
              ...
              >
                <label htmlFor="workspaceAvailability">
          ...
          {selectedReport ===
            ...
            >
              ...
              >
                <label htmlFor="workspaceHoldOfficeCode">
                  Office code (optional)
                </label>
                <input
                  id="workspaceHoldOfficeCode"
