...
async function getLookupOptions(model: {
...
export async function getWorkspaceCategories() {
  return getLookupOptions(prisma.workspaceCategory);
}

export async function getDeskTypes() {
  return getLookupOptions(prisma.deskType);
}

export async function getWorkspaceAssignmentTypeNameById(id: number) {
  const workspaceAssignmentType =
    await prisma.workspaceAssignmentType.findUnique({
      where: {
        id,
      },
      select: {
