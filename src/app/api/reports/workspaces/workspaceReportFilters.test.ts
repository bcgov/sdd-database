...
assert.deepEqual(freeUnassigned, {AND: [{office_number: "A-100"}, {employee_id: null}, {is_on_hold: false}]});

const onHoldUnassigned = buildWorkspaceReportWhereClause({officeCode: "", availability: "onhold", mode: "unassigned"});
assert.deepEqual(onHoldUnassigned, {AND: [{employee_id: null}, {is_on_hold: true}]});
